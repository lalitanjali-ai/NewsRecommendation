...
#     st.title("News Recommendation Engine")
#
#     option = st.radio("Choose an option:", ("User ID", "Article ID"))
#
#     if option == "User ID":
#         user_id = st.number_input("Enter a user ID:", min_value=1, step=1)
#
...
#             st.write("Scores:", result_dict["score"])
#
#     elif option == "Article ID":
#         article_id = st.number_input("Enter an article ID:", min_value=1, step=1)
#
#         if st.button("Get Similar Articles"):
#             ref_title, similar_titles, similarity_prob = results.get_similarity(ix=article_id, k=5)
#
#             st.header("Reference Article")
#             st.write(ref_title.values[0])
#
#             st.header("Similar Articles")
#             for i, title in enumerate(similar_titles):
#                 st.write(title.values[0])
#                 st.write("Similarity score:", similarity_prob[i])